...
class AgentTracer {
    ...
    endSession(sessionId: string, success: boolean): AgentTrace | undefined {
        ...
        if (trace) {
            ...
            Zotero.debug(`[seerai][trace] Duration: ${trace.totalDurationMs.toFixed(0)}ms`);
            Zotero.debug(`[seerai][trace] Iterations: ${trace.iterations.length}`);
            Zotero.debug(`[seerai][trace] Tool calls: ${trace.totalToolCalls} (${trace.failedToolCalls} failed)`);
    ...
    getExecutionSummary(trace: AgentTrace): string {
        ...
        for (const iteration of trace.iterations) {
            for (const span of iteration.toolSpans) {
                const existing = toolTimes.get(span.toolName) || { count: 0, totalMs: 0 };
                existing.count++;
