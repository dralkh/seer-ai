...
export async function executeGetItemMetadata(
    ...
    try {
        ...
        const result: GetItemMetadataResult = {
            ...
            authors: creators,
            year: (item.getField("year") || item.getField("date")?.toString().substring(0, 4) || "") as string,
            ...
            publication: (item.getField("publicationTitle") || item.getField("bookTitle") || undefined) as string | undefined,
...
export async function executeReadItemContent(
    ...
    try {
        ...
        if (item.itemType === "note") {
            ...
            return {
                ...
                data: {
                    ...
                    truncated: (max_length || config.maxContentLength) > 0 && content.length >= (max_length || config.maxContentLength),
                },
        ...
        let notesCount = item.getNotes().length;
        ...
        if ((!content || content.trim().length < 100) && (trigger_ocr || config.autoOcr) && include_pdf) {
            Zotero.debug(`[seerai] No content found, triggering OCR for item ${item.id}`);
            ...
            if (pdf) {
                ...
                // Retry getting text
                const ocrText = await Assistant.getPdfTextForItem(item, max_length || config.maxContentLength, true, true);
                if (ocrText) {
                    content = ocrText;
