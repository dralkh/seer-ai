...
import { ChatMessage, ChatStates, ChatOptions, ConversationMetadata } from "./types";
...
export abstract class MessageStore {
    ...
    abstract updateConversationMetadata(metadata: Partial<ConversationMetadata>): Promise<void>;
}
...
export class FileMessageStore extends MessageStore {
    ...
    private get messagesFile(): string {
        return PathUtils.join(this.conversationsDir, this.currentConversationId, "messages.jsonl");
    }

    private get stateFile(): string {
        return PathUtils.join(this.conversationsDir, this.currentConversationId, "state.json");
    }
    ...
    private async migrateLegacyStorage(): Promise<void> {
        try {
            ...
            const legacyState = PathUtils.join(this.dataDir, "conversation_state.json");
            ...
            if (await IOUtils.exists(legacyMessages)) {
                ...
                await IOUtils.move(legacyMessages, PathUtils.join(defaultDir, "messages.jsonl"));
                if (await IOUtils.exists(legacyState)) {
                    await IOUtils.move(legacyState, PathUtils.join(defaultDir, "state.json"));
                }
    ...
    public async loadMessages(): Promise<ChatMessage[]> {
        ...
        try {
            ...
            const convDir = PathUtils.join(this.conversationsDir, this.currentConversationId);
    ...
    public async updateConversationMetadata(metadata: Partial<ConversationMetadata>): Promise<void> {
        try {
            const history = await this.getHistory();
            const idx = history.findIndex(h => h.id === (metadata.id || this.currentConversationId));
    ...
    private async updateHistoryFromMessage(message: ChatMessage): Promise<void> {
        ...
        await this.updateConversationMetadata({
            preview: message.content.slice(0, 100).replace(/\n/g, " ") + (message.content.length > 100 ? "..." : "")
        });
    }
}
