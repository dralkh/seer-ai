...
class FirecrawlService {
     ...
     */
    async webSearch(query: string, limit: number = 5): Promise<FirecrawlSearchResult[]> {
        ...
        try {
            ...
            const data = await response.json() as unknown as FirecrawlSearchResponse;
    ...
    async researchSearch(
        ...
        try {
            ...
            Zotero.debug(`[seerai] Firecrawl research returned ${results.length} results`);
    ...
    private extractPdfFromResults(
        ...
        for (const result of results) {
            ...
            if (result.markdown) {
                ...
                if (downloadMatch) {
                    Zotero.debug(`[seerai] Found download link in markdown: ${downloadMatch[2]}`);
                    return {
